...
const SidebarTest = () => {
  ...
  const Menus = [
    ...
    { title: "Riwayat Transaksi", src: "Search", path: "/riwayatTransaksiCust", },
  ...
  return (
    <div className='flex'>
      <div className={`${open ? "w-72" : "w-20"} bg-teal-color h-screen p-5 pt-8 relative duration-400`}>
        ...
        <div className='flex gap-x-4 items-center'>
          ...
          <h1 className={`text-dark-purple origin-center font-medium text-xl duration-200 ${!open && "scale-0"}`}>Grand Atma Hotel</h1>
        </div>
        <ul className='pt-6'>
        ...
        {Menus.map((Menu, index) => {
            ...
            } else if (showMenu === 'Sales Marketing' && (index === 0 || index === 1 || index === 3 || index === 5 || index === 4)) {
              return (
                <li key={index}
                  className={`flex rounded-md p-2 cursor-pointer 
                hover:bg-light-white 
